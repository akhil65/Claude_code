...
Generate a complete architecture report for this project by following every step below. $ARGUMENTS overrides the output directory (default: `docs/architecture`).
...
## Step 2 — Generate four Mermaid diagrams
...
Write the following four `.mmd` files. Tailor each diagram to what actually exists in this project — do not invent layers or components that are absent. Keep each diagram to 15–25 nodes and use clear, concise labels.
...
Render all four diagrams with the dark theme:

```bash
cd docs/architecture
mmdc -i frontend.mmd     -o frontend.svg     -t dark
mmdc -i backend.mmd      -o backend.svg      -t dark
mmdc -i dataflow.mmd     -o dataflow.svg     -t dark
mmdc -i dependencies.mmd -o dependencies.svg -t dark
```
